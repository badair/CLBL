...
struct assert_generalizations {
    ...
    static_assert(assert_same<
        typename generalized_object<T>::type,
        typename generalized_object<T*>::type,
        typename generalized_object<std::reference_wrapper<T>>::type
    >{}, "");
    ...
    template<typename U>
    using generalize_call_result = decltype(generalize<U>(std::declval<U>()));
