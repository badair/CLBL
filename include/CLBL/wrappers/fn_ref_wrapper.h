...
#ifndef CLBL_FN_REF_WRAPPER_H
#define CLBL_FN_REF_WRAPPER_H
...
namespace clbl {
    ...
    struct fn_ref_wrapper { static_assert(sizeof(Failure) < 0, "Not a function."); };

    template<typename Creator, typename Return, typename... Args>
    struct fn_ref_wrapper<Creator, Return(Args...)> {
        ...
        using my_type = fn_ref_wrapper<Creator, Return(Args...)>;
