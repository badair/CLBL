...
namespace clbl {
    ...
    template<typename, qualify_flags, typename, typename, typename TMemberFnPtr, typename DispatchFailureCase>
    ...
    struct casted_fn_obj_ptr_wrapper<Creator, CvFlags, UnderlyingType, TPtr, TMemberFnPtr, Return(std::remove_cv_t<UnderlyingType>::*)(Args...)> {

        using args_t = std::tuple<Args...>;
        using clbl_tag = pmf_ptr_tag;
        using creator = Creator;
        using forwarding_glue = Return(forward<Args>...);
        using invocation_data_type = object_pointer_casted_invocation_data<TPtr, TMemberFnPtr>;
        using my_type = casted_fn_obj_ptr_wrapper<Creator, CvFlags, UnderlyingType, TPtr, TMemberFnPtr, Return(std::remove_cv_t<UnderlyingType>::*)(Args...)>;
        using return_t = Return;
        using type = Return(Args...);
        using underlying_type = clbl::underlying_type<UnderlyingType>;

        template<qualify_flags Flags>
        using apply_cv = casted_fn_obj_ptr_wrapper<Creator, CvFlags | Flags, UnderlyingType, TPtr, TMemberFnPtr, Return(std::remove_cv_t<UnderlyingType>::*)(Args...)>;
