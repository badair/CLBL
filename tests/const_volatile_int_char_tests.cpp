...
void const_volatile_int_char_tests() {
    ...
    TEST(f(1, 'c') == test_id::const_volatile_int_char_struct_op);

    auto g = func(&int_char_func);
    TEST(g(1, 'c') == test_id::int_char_func);

    auto h = func(&const_volatile_int_char_object, &decltype(const_volatile_int_char_object)::func);
    TEST(h(1, 'c') == test_id::const_volatile_int_char_struct_func);
